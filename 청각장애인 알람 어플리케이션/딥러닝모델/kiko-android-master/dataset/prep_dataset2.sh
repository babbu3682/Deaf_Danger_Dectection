...
if [ -d ./class ]; then
  ...
  if [ ! -d ./class/speech ]; then
    ...
    echo "Taking 50 samples from each of the 30 speech_commands classes..."    # 다른 yes,or,no를 모아서 speech 데이터 셋을 만듬.
  ...
  for wavfile in class/*/*; do
    echo $wavfile
    ffmpeg -y -loglevel -8 -i $wavfile -ac 1 -af aresample=resampler=soxr -ar $SAMPLING_RATE $wavfile 2>&1 >/dev/null # 여기가 가장 중요
  done
